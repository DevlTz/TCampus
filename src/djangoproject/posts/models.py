...
class PostsManager(models.Manager):
    def get_feed_for_user(self, user):
        ...
        return (
            self.get_queryset()
            .filter(postedBy__in=user.following.all())
            .order_by("-postedAt")
        )
...
class EventsManager(models.Manager):
    def get_feed_for_user(self, user, scope="following"):
        ...
        if scope == "all":
            ...
            return (
                self.get_queryset().filter(event_date__gte=now).order_by("-event_date")
            )
        else:
...
class Posts(models.Model):
    ...
    postedBy = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="posts_postedBy", editable=False
    )
...
class Events(models.Model):
    ...
    postedAt = models.DateTimeField(auto_now_add=True, editable=False)
    postedBy = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="events_postedBy", editable=False
    )
    objects = models.Manager()
    participants = models.ManyToManyField(
        User, related_name="events_participants", blank=True
    )
